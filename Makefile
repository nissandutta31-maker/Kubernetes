.PHONY: help build kind-up kind-down load-image deploy verify port-forward clean all
...
load-image: build ## Load the local Docker image into the Kind cluster
...
verify: ## Run the Python health-check script against the cluster
	python3 automation/health_check.py --namespace $(NAMESPACE)
...
# ── All-in-one ──────────────────────────────────────────────────
all: kind-up load-image deploy verify ## Full demo: cluster → build → load → deploy → verify
	@echo ""
