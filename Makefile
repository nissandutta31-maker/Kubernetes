.PHONY: help build kind-up kind-load kind-down deploy verify port-forward clean all
...
kind-load: ## Load the local Docker image into the Kind cluster
...
# ── All-in-one ──────────────────────────────────────────────────
all: kind-up build kind-load deploy verify ## Full demo: cluster → build → load → deploy → verify
	@echo ""
