...
func (r *RuntimePackageReconciler) patchStatus(
	...
	if installedVersion != "" {
		...
		phase == runtimev1alpha1.PackagePhasePending {
		// Clear InstalledVersion when no nodes match (Pending) or installation
		// is broken (Failed). Installing is intentionally excluded: a stale
		// InstalledVersion is needed so the Upgrading case can detect that a
		// version rollout is in progress (InstalledVersion != deployedVersion).
